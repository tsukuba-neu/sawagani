...
![Googleスプレッドシートのテーブルをクリップボードにコピーし、Webブラウザで開いたアプリケーションにインポートしてPDF出力する様子](/docs/quick_tour.gif)
...
- **📄 出力後のPDFファイルからの再インポートができます。**
  - ブラウザの印刷機能から出力したPDFを「ファイルからインポート」ボタンから読み込むと、データを読み込んで編集することができます。
  - PDFにしたあとで修正の必要が生じても、再度編集することができます。また、提出前のPDFそのものを作業データとして補完することができます。
  - この機能では、出力されるPDFの最後のページに載っている二次元コードを読み取ることで実現しています。提出する際にはこのページを抜き取ってください。
